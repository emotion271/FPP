...
class FashionNet(nn.Module):
    ...
    def scores(self, ulatent, ilatents, ifeat, scale=10.0):
    ...
    def _pairwise_output(self, lcus, pos_feat, neg_feat, encoder):
        ...
        pscore, lpi = self.scores(lcus, lcpi_f, feat_comp)
        nscore, lni = self.scores(lcus, lcni_f, feat_comn)

        # pscore, lpi = self.scores(lcus, lcpi_s, lcpi_s)
        # nscore, lni = self.scores(lcus, lcni_s, lcni_s)
    ...
    def forward(self, *inputs):
        ...
        if self.param.use_semantic and self.param.use_visual:
            ...
            scores, _, dloss_o = self.visual_output(lcus_f, posi, nega)
...
class FashionNetDeploy(FashionNet):
    ...
    def _single_output(self, lcus, feat, encoder):
        ...
        #assert(0==1)
        '''
        score, _ = self.scores(lcus, lcpi_s, lcpi_s)
        # score with binary codes
        #bcus = self.sign(lcus)
        #bcpi = [self.sign(h) for h in lcpi]
    ...
    def forward(self, *inputs):
        ...
        if self.param.use_semantic and self.param.use_visual:
            ...
            score = self.visual_output(lcus_eb, items)
        elif self.param.use_semantic:
